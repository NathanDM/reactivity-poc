...
function Dashboard({ addSticky, updateSticky, removeSticky }) {
  return (
    <div className="dashboard">
      ...
      <button className="button" disabled="disabled" onClick={() => { removeSticky(); }}>Remove</button>
      <button className="button" disabled="disabled" onClick={() => { updateSticky(); }}>Update</button>
    </div>
  );
}
